...
def station_mac_dict_list_get():
    ...
    return stat_list
# 返回所有存在的站点名称和MAC地址字典的列表 +新设备
def station_mac_dict_list_get_new():
    sqlcmd = "select site_name as station,mac from bcell_devices where site_name is not null;"
    stat_list = sqlcmd_excute(sqlcmd, "dict")
    return stat_list
...
def search_mac_list_by_station_new(site_name=None, address=None):
    ...
    if site_name:
        if address:
            sqlcmd = "select mac from bcell_devices where site_name='%s' and site_address_area='%s';" \
                     % (site_name, address)
        else:
            sqlcmd = "select mac from bcell_devices where site_name='%s';" % (site_name)
    else:
        sqlcmd = "select mac from bcell_devices where site_name is not null;"
...
def follow_analysis(imsi, time_from, time_end, station_list=[], capture_interval=900):
    ...
    if station_list:
        for i in station_list:
            mac = search_mac_list_by_station(i)[0]
            device_list.append({"station": i, "mac": mac})  # 将搜索结果添加到原列表中每项(字典)中去
    ...
    # 返回搜索结果
    return overview_dict, main_list, detail_list,sql_info
# 伴随分析 +new 新设备
def follow_analysis_new(imsi, time_from, time_end, station_list=[], capture_interval=900):
    sql_info=[]
    # 判断输入的关键参数存不存在，若不存在则无法搜索，直接返回空
    if (not imsi) or (not time_from) or (not time_end):
        return {}, [], []
    ...
    if station_list:
        for i in station_list:
            mac = search_mac_list_by_station_new(i)[0]
            device_list.append({"station": i, "mac": mac})  # 将搜索结果添加到原列表中每项(字典)中去
    else:  # 若未指定站点列表，则默认所有的站点进行查询
        device_list = station_mac_dict_list_get_new()
    ...
    try:
        ...
        for d in device_list:
            # 先将目标IMSI的数据先找到
            sqlcmd = "select capture_time from bcell_imsi where capture_time>='%s' and capture_time<'%s' and imsi='%s' ;" \
                     % (time_from, time_end, imsi)
            sql_info.append(sqlcmd)
            cur.execute(sqlcmd)
            data_list = cur.fetchall()
            ...
            # bcell_imsi,bcell_devices表 无phone_num
            # # 当初次查询到目标IMSI数据时，查询更新其归属地信息
            # if site_flag["val"]:
            #     for data in data_list:  # 为了防止有的phone_num 字段为空
            #         if data['phone_num']:
    ...
    # 返回搜索结果
    # return overview_dict, main_list, detail_list,sql_info
    return overview_dict, device_list, detail_list,sql_info
...
def permanent_analysis(station, time_from, time_end, capture_interval=900):
    ...
    try:
        ...
        sqlcmd = "create temporary table %s(imsi char(16), phone_num char(16), slice_time_from char(20), slice_time_end char(20))default charset=utf8;" % (
            tmp_table)
        cur.execute(sqlcmd)
        ...
        while from_seconds < end_seconds:
            ...
            sqlcmd = "insert into %s(imsi, phone_num, slice_time_from, slice_time_end) (select distinct imsi,phone_num,'%s' as time_slice_from, '%s' as time_slice_end from imsi_%s where capture_time>='%s' and capture_time<='%s');" \
                     % (tmp_table, from_str, end_str, mac, from_str, end_str)
            cur.execute(sqlcmd)
            # 起始时间戳更新，必须做
        ...
        for i in main_list:
            ...
            # 补全IMSI总和信息
            sqlcmd = "select count(1) as imsi_count from imsi_%s where imsi='%s';" % (mac, i['imsi'])
            cur.execute(sqlcmd)
            tmp_list = cur.fetchall()
            if tmp_list:
                i.update(tmp_list[0])
    ...
    # 返回搜索结果
    return overview_dict, main_list
