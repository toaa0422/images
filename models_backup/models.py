...
def station_mac_dict_list_get_new():
    sqlcmd = "select site_name as station,mac,sn from bcell_devices where site_name is not null;"
    stat_list = sqlcmd_excute(sqlcmd, "dict")
    return stat_list
...
# 参数：site_name:站点名称, address:站点地址,sn:设备
# 返回：返回所有站点的MAC地址列表
...
def follow_analysis_new(imsi, time_from, time_end, station_list=[], capture_interval=900):
    ...
    if station_list:
        for each in station_list:
            try:
                mac, sn = search_mac_list_by_station_new(each)
                if mac or sn:
                    device_list.append({"station": each, "mac": mac, "sn": sn})  # 将搜索结果添加到原列表中每项(字典)中去
            except:
                continue
    else:  # 若未指定站点列表，则默认所有的站点进行查询
        # return {},[],[]
