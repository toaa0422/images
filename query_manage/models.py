...
def follow_analysis(imsi, time_from, time_end, station_list=[], capture_interval=900):
    ...
    if station_list:
        for each_station in station_list:
            ...
            sqlcmd="select site_name as station,mac,sn,rpt_time from bcell_devices where site_name='%s';"%(each_station)
        ...
        for d in device_list:
            ...
            if 'sn' not in d:
                ...
                for i in data_list:
                    ...
                    tmp_end_str = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(tmp_end_second))
                    # 将搜索到的数据存入临时表中
                    sqlcmd = "insert into %s(mac, station, aim_capture_time,imsi, capture_time, rssi, rpt_time) (select distinct '%s' as mac,'%s' as station,'%s' as aim_capture_time, imsi,capture_time,rssi, '%s'as rpt_time from bcell_imsi  where capture_time>='%s' and capture_time<'%s' and imsi!='%s' and sn = '%s');" \
                             % (tmp_table, d['mac'], d['station'],i['capture_time'],d['rpt_time'], tmp_from_str, tmp_end_str,
                                imsi,d['sn'])
    ...
    try:
        ...
        if "sn" not in device_list[0]:
            ...
            while from_seconds < end_seconds:
                ...
                overview_dict["slice_sum"] += 1  # 切片数据量加一
        # 取出临时表中数据
        sqlcmd = "select imsi,count(imsi) as time_count,sn from %s group by sn,imsi having time_count>'%d' order by time_count desc limit 500;" % (
        tmp_table, overview_dict["slice_sum"] // 3)
        sql_info.append(sqlcmd)

        cur.execute(sqlcmd)
        main_list = cur.fetchall()
        sql_info.append(main_list)
        # 补全其他信息
        for i in main_list:
            if not i['sn']:
            # 补全归属地信息
                ...
                # 补全IMSI总和信息
                sqlcmd = "select count(1) as imsi_count from imsi_%s where imsi='%s';" % (device_list[0]['mac'], i['imsi'])
                sql_info.append(sqlcmd)
                ...
                # 补全新设备IMSI总和信息
                sqlcmd = "select count(1) as imsi_count from bcell_imsi where imsi='%s' and sn='%s';" % (i['imsi'],i['sn'])
                sql_info.append(sqlcmd)
